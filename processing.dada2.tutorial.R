...
path <- "~/projects/dada2_tutorial_16S/raw_data/" # CHANGE ME to the directory containing the fastq files
list.files(path)
...
numplot <- 5 #CHANGE ME to the number of samples you want to include in the quality plot ( can be anywhere from 2 to length(fnFs) )
...
filterAndTrim(fnFs, fnFs.filtN, fnRs, fnRs.filtN, trimLeft = c(0,0), trimRight = c(0,0), truncLen=c(270,170), maxN = 0, multithread = 32, compress = TRUE, matchIDs=TRUE)
...
filtFs <- sort(list.files("~/projects/dada2_tutorial_16S/raw_data/cutadapt/filtered/", pattern = "_R1_001.fastq.gz", full.names = TRUE)) #remember to change the pattern so it matches ALL your file names!
filtRs <- sort(list.files("~/projects/dada2_tutorial_16S/raw_data/cutadapt/filtered/", pattern = "_R2_001.fastq.gz", full.names = TRUE)) #remember to change the pattern so it matches ALL your file names!
...
samples_to_keep <- track[,4] > 1 #your threshold. try different ones to get the lowest one that will work. #this method accounts for dereplication/ASVs left after inference
...
cat("", file="ASV_report.txt", sep="\n", append=TRUE)
cat("dimensions of ASV table after singleton removal:", dim(otus_filt),file="ASV_report.txt",sep="\t",append=TRUE)
cat("", file="ASV_report.txt", sep="\n", append=TRUE)
...
row.names(out) <- as.character(t(as.data.frame(strsplit(row.names(out), "_")))[,1]) #make sure that the output you're assigning to the row.names of "out" matches your sample names before proceeding. select the correct delimiter (_ in exampe) and element (1 in example) to extract a unique name for each sample that corresponds to any metadata you will work with in downstream analyses
getN <- function(x) sum(getUniques(x))
track <- cbind(out[which(row.names(out) %in% names(samples_to_keep)[which(samples_to_keep == TRUE)]),], sapply(dadaFs[samples_to_keep], getN), sapply(dadaRs[samples_to_keep], getN), sapply(mergers, getN), rowSums(seqtab.nosingletons), rowSums(seqtab.nosingletons.nochim))
...
tax_info_16S <- IdTaxa(seq_16S, trainingSet, strand = "both", threshold=40, processors = 38) #this is faster than other methods we've used before. with 38 processors, it only took about 15 min
...
write.table(tax_tab_16S, "taxonomy_table.16S.DECIPHER.GTDB.threshold_40.txt", sep = "\t", quote = F, row.names = FALSE)
...
write.table(data.frame("row_names"=rownames(seqtab.nosingletons.nochim),seqtab.nosingletons.nochim),"sequence_table.16S.merged.w_ASV_names.txt", row.names=FALSE, quote=F, sep="\t")
...
taxa_boot <- assignTaxonomy(seqtab.nosingletons.nochim, "/data/taxonomyDBs/silva_for_dada2/v138_by_dada2team/silva_nr99_v138.1_train_set.fa.gz", multithread=38, outputBootstraps = TRUE)
taxa <- addSpecies(taxa_boot[[1]], "/data/taxonomyDBs/silva_for_dada2/v138_by_dada2team/silva_species_assignment_v138.1.fa.gz") #here we operate on the first element of the list taxa_boot, the character matrix. the second element is a numeric matrix of confidence values up to genus equivalent taxonomic rank
...
#re-save sequence and taxonomy tables with updated names
write.table(data.frame("row_names"=rownames(taxa),taxa),"taxonomy_table.16S.RDP_SILVA138_dada2.txt", row.names=FALSE, quote=F, sep="\t")
write.table(data.frame("row_names"=rownames(taxa_boot$boot),taxa_boot$boot),"taxonomy_table.16S.RDP_SILVA138_dada2.bootstraps.txt", row.names=FALSE, quote=F, sep="\t")     


####taxonomy assignment with NCBI NT blast db and LCA####
#this is example code, you need a local copy of NCBI's NT database formatted for use with blast, which you can download from NCBI's FTP website
#depends on: blastn, galaxy-tool-BLAST (find on github), and galaxy-tool-LCA (find on github)
#now assign taxonomy with blast NT database at 96% similarity threshold using both 'LCA + besthit' and 'LCA only' parameters
mkdir blast_96_sim_LCA_besthit
blastn -task megablast -num_threads 38 -evalue 1e-5 -max_target_seqs 10 -perc_identity 96 -qcov_hsp_perc 50 -db /data/taxonomyDBs/NCBI_NT/2023-06-08/nt -outfmt '6 qseqid stitle sacc staxid pident qcovs evalue bitscore' -query 16S_ASV_sequences.fasta  -out blast_96_sim_LCA_besthit/16S_ASV_sequences.blast.out
python2 ~/programs/galaxy-tool-BLAST/blastn_add_taxonomy_lite.py -i blast_96_sim_LCA_besthit/16S_ASV_sequences.blast.out -t /data/taxonomyDBs/NCBI_taxonomy/2023-06-08/rankedlineage.dmp -m /data/taxonomyDBs/NCBI_taxonomy/2023-06-08/merged.dmp -o blast_96_sim_LCA_besthit/taxonomy
cat <(head -n 1 ~/programs/galaxy-tool-lca/example/example.tabular) taxonomy_16S_ASV_sequences.blast.out > tmp
python2 ~/programs/galaxy-tool-lca/lca.py -i tmp -o blast_96_sim_LCA_besthit/taxonomy_table.16S.NCBI_NT.96sim.LCA+besthit.txt -b 100 -id 96 -cov 50 -t best_hit -tid 98 -tcov 80 -fh environmental,unidentified,kingdom -flh unclassified

mkdir blast_96_sim_LCA_only
python2 ~/programs/galaxy-tool-lca/lca.py -i tmp -o blast_96_sim_LCA_only/taxonomy_table.16S.NCBI_NT.96sim.LCA_only.txt -b 100 -id 96 -cov 50 -t only_lca -fh environmental,unidentified -flh unclassified
